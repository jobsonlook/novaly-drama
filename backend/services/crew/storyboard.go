...
- 动作、视线和反应对象必须写具体角色姓名；禁止写“听者、对方、对面的人、另一人、其眼神/态度”。即使前文能推断，也必须明确写成“洛清霜盯住韩铮”“韩铮等待洛清霜回答”。对白近景/中近景/特写还必须根据刚发生的刺激写 1～2 个可见表演信号及其变化，不能只写开口、皱眉或冷笑。
- 同一角色的一次完整发言优先留在同一条 10 秒分镜；单拍装不下时先延续到本镜下一拍并换景别，只有整镜超过对应容量才跨镜。后半拍禁止复读已说完的内容。
...
细切时一条优先承载同一角色的一次完整发言；可在【3-7秒】【7-10秒】继续原发言，但不要顺带塞入另一角色的新发言。
...
- 必须切下一条的边界只有：换场/换空间、闪回进入或回到现在、时间跳跃、换装完成态切换、当前 10 秒按分类语速已经装不下。
- 禁止按「说话人乒乓 / 情绪微变 / 喝水三步骤」拆成碎镜。`
	}
	return `按「细切」拆：一条分镜优先承载同一角色的一次完整发言，或一个关键动作。同一剧本多次拆镜，条数必须落在这个区间。

【切镜边界 · 细切（对齐第1集）】
- 换说话人 → 新开一条。禁止把两句完整对白塞进同一条。
- 一次发言在【0-3秒】说不完时，先按自然语义停顿延续到本镜【3-7秒】【7-10秒】并换景别。普通对白整镜建议 40–45 字、上限 50 字；快速对白极限 55 字。只有整镜确实装不下才拆到下一镜。

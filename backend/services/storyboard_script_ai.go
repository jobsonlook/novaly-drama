...
R2 台词：写成 阿彪说：「……」。禁止空「」。同一句不要复读。同一角色的一次完整发言优先留在当前10秒分镜；单拍装不下，按自然语义停顿延续到本镜下一拍并换景别，只有整镜装不下才跨镜。普通对白整镜建议40–45字、上限50字，快速对白极限55字。分类语速：普通4.5–5字/秒，快速5–6，萌宝4–4.5，威胁/揭晓3.5–4.5，喘息/哭腔3–4。禁止精简原文、改派说话人或编造对白。
R3 人物目标、表演与因果：每句台词和动作必须承接上一拍的具体刺激，服务于人物当下目标。对白近景/中近景/特写中的说话人不能只写开口；从眉眼、眨眼频率、视线停留/闪避、嘴角、抿唇、咬肌、下颌、吞咽、呼吸中选择1～2个可见信号。一句发言跨多拍时，表情要有因果阶段变化，例如强装镇定→被证据戳中后闪避→重新抬眼嘴硬。禁止只贴「生气、震惊、开心」标签，禁止每拍复制皱眉/冷笑，禁止所有角色套同一表情。远景、背影、纯物件、高速动作不强塞面部表情，改用肩颈、手部力度和呼吸。动作、视线和反应对象必须写具体角色姓名，不得用停顿或沉思凑时长。
R4 衍生态绑衍生图，穿衣绑日常父角色。
...
【每拍必备信息】
每个【秒】行都要形成确定、可拍的画面，并包含：景别与运镜；人物位置与朝向（适用时）；承接上下文的具体动作与目标；面部可读时加入剧情驱动的微表情，面部不可读时用身体表演；配乐床、环境声及动作声；本拍台词（如有）。禁止把规则、备选项或解释文字写进文案。
每拍自检：承接了什么信息 → 人物想得到什么 → 做了什么可见动作 → 下一拍需要回应什么。任何一项不明确就重写。

【台词铁律】
1. 零删改：禁止精简、合并、改派说话人。禁止新编对白、禁止现代网络梗/弹幕吐槽。
2. 按分类语速计时：普通4.5–5字/秒，快速5–6，萌宝4–4.5，威胁/揭晓3.5–4.5，喘息/哭腔3–4；标点不计。
3. 同一角色完整发言优先留在当前10秒分镜；单拍装不下先延续到本镜下一拍并换景别。普通对白整镜上限50字，快速对白极限55字；整镜装不下才跨镜。
4. 一拍一个连续对白片段；切换说话人时换拍。同一句不要抄两遍。
5. 必须标明说话人。无台词不要写空「」。
